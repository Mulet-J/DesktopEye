...
using LanguageHelper = DesktopEye.Common.Services.Helpers.LanguageHelper;

namespace DesktopEye.Common.Services.TextClassifier;

public class NTextCatClassifierService : ITextClassifierService
{
        ...
        ILogger<NTextCatClassifierService> logger)
    {
        ...
        // var downloadResult = DownloadModel().Result;
        //
        // if (!downloadResult)
        // {
    ...
    public List<(Language language, double confidence)> ClassifyTextWithProbabilities(string text)
    {
        ...
        try
        {
            //TODO normaliser distances -> probabilitées
            var languages = _languageIdentifier.Identify(text);
            ...
            if (_logger.IsEnabled(LogLevel.Trace))
                foreach (var (language, confidence) in results.Take(3)) // Log top 3 results
    ...
    private Language LibLanguageToLanguage(string? language)
    {
        ...
        var result = LanguageHelper.FromIso3ToLanguage(language);
